...
def set_name():  # event is passed by binders.
    ...
    if msg == "":
        messagebox.showwarning('Erro', 'Você não digitou um remetente(nome) para se comunicar no chat')   
    else:
        e_assunto.config(state='normal')
        e_destinatario.config(state='normal')
        e_mensagem.config(state='normal')
        e_remetente.config(state='disabled')
...
def send():
    ...
    if destinatario.get() != "" and mensagem.get() != "":
        ...
        assunto.set("") # limpa o campo de assunto
...
e_destinatario = tkinter.Entry(window, font="Ubuntu 12 bold", fg="#483659", textvariable=destinatario, state='disabled')
e_destinatario.bind("<Return>", )
e_assunto = tkinter.Entry(window, font="verdana 12 bold", fg="#483659", textvariable=assunto, state='disabled')
e_assunto.bind("<Return>", )
e_mensagem = tkinter.Entry(window, font="Ubuntu 12 bold", fg="#483659", width=65, textvariable=mensagem, state='disabled')
